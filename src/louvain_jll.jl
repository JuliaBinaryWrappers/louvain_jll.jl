...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import louvain_jll on Julia versions older than 1.3!")
end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("louvain_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "louvain_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"louvain", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "louvain_jll", "src", "louvain_jll.jl"))
        @info("louvain_ll dev'ed out to /depot/dev/louvain_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("3a01c810-c4d4-5434-a337-635ca50da98f"))
platforms = [Pkg.Artifacts.unpack_platform(e, "louvain", artifacts_toml) for e in artifacts["louvain"]]
...
if best_platform === nothing
    @debug("Unable to load louvain; unsupported platform $(triplet(platform_key_abi()))")
else
